...
mod tests {
    ...
    #[test]
    fn test_magic() {
        let mut file = File::open("/home/extraymond/Downloads/takes/001.c3d").unwrap();
        ...
        file.seek(SeekFrom::Start((header.parameter_start as u64 - 1) * 512))
            .unwrap();
        dbg!(header.parameter_start);
        ...
        unsafe {
            ...
            (&buffer[..]).read_exact(parameter_slice).unwrap();
        }
        ...
        parameter_buf.resize(parameter.parameter_block_counts as usize * 512 - 4, 0);
        file.read_exact(&mut parameter_buf).unwrap();
        ...
        for (k, v) in groups.iter() {
            for (p_name, p) in v.params.iter() {
                println!("{}: {}", &v.name, &p_name);
            }
        }
    }
}
